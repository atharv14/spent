          ...
          <div className="flex flex-col items-center gap-8">
            ...
            {isLoading ? (
              <div className="w-full text-center">
                <div className={`animate-spin rounded-full h-8 w-8 border-b-2 mx-auto ${theme === 'dark' ? 'border-white' : 'border-gray-900'
                  }`} />
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                {items.map((item, index) => (
                  <div
                    key={index}
                    className={`p-4 rounded-lg border hover:shadow-lg transition-shadow overflow-hidden ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'
                      }`}
                  >
